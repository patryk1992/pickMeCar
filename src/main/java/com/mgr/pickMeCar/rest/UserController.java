...
public class UserController {
	...
	private UserDAO userDao;
	 @RequestMapping(value = "/rest-api/user",method = RequestMethod.GET,produces = "application/json",headers="Accept=*/*")
	    public ResponseEntity <List<User>> listMark() {
			
			List<User> liUsers =userDao.list();
			logger.info("Welcome list"+ liUsers.toString());
//			liUsers.get(0).setUser(null);
//			if(liMark.isEmpty()){
...
//			Mark test = new Mark(usr, 4, "dsag");
	        return new ResponseEntity <List<User>>(liUsers, HttpStatus.OK);
	 }
	 @RequestMapping(value = "/rest-api/user",method = RequestMethod.POST,produces = "application/json",headers="Accept=*/*")
	 ...
	 @RequestMapping(value = "/rest-api/user/{id}",method = RequestMethod.DELETE,produces = "application/json",headers="Accept=*/*")
